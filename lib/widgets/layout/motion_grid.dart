...
class _RenderMotionGrid extends RenderGrid {
  _RenderMotionGrid({
    ...
    _controller = AnimationController(vsync: vsync, duration: duration)
      ..addListener(() {
        if (_extentFrom != _extentTo && !_layingOut) {
          markNeedsLayout();
        }
        _placeChildren();
        markNeedsPaint();
      ...
      ..addStatusListener((status) {
        if (status.isCompleted && _extentFrom != _extentTo) {
          _extentFrom = _extentTo;
          markNeedsLayout();
        }
      });
  }
  ...
  /// A grid that loses its last row shrinks over the motion rather than at
  /// once, so a scroll view pinned to its end follows instead of jumping.
  ...
  double _mainExtentOf(Size size) =>
      mainAxis == Axis.vertical ? size.height : size.width;
  ...
  void _layOut() {
    ...
    for (final child in getChildrenAsList()) {
      ...
      if (parentData.target != null) {
        painted[child] = parentData.offset;
      }
      targets[child] = parentData.exiting ? null : parentData.target;
    }
    final paintedExtent = hasSize ? _mainExtentOf(size) : null;
    super.performLayout();
    final laidOutExtent = _mainExtentOf(size);
    ...
    for (final child in getChildrenAsList()) {
      ...
      if (parentData.exiting) {
        ...
        parentData
          ..target = painted[child] ?? parentData.offset
          ..from = Offset.zero;
      } else {
        parentData.exitSize = null;
      }
    }
    // Relaid out mid-motion with nothing new to reach, the motion carries on.
    final retarget =
        !animate ||
        (laidOutExtent - _extentTo).abs() > precisionErrorTolerance ||
        getChildrenAsList().any((child) {
          final parentData = _parentDataOf(child);
          return !parentData.exiting && targets[child] != parentData.offset;
        });
    if (!retarget) {
      _placeChildren();
      _applyExtent();
    ...
    final shrinks =
        ...
        paintedExtent != null &&
        paintedExtent > laidOutExtent + precisionErrorTolerance;
    _extentFrom = shrinks ? paintedExtent : laidOutExtent;
    _extentTo = laidOutExtent;
    var moving = shrinks;
    for (final child in getChildrenAsList()) {
      final parentData = _parentDataOf(child);
      if (parentData.exiting) {
  ...
  void _applyExtent() {
    if (_extentFrom == _extentTo) {
      return;
    }
    final t = _curve.transform(_controller.value).clamp(0.0, 1.0);
    final extent = _extentFrom + (_extentTo - _extentFrom) * t;
    size = constraints.constrain(
      mainAxis == Axis.vertical
          ? Size(size.width, extent)
          : Size(extent, size.height),
    );
  }
