...
/// Unwrap a deep link, or an http(s) URL wrapping one; a share link comes back
/// as data. Null when neither; [IncyLinkException] when ours but unresolvable.
Future<ResolvedExternalLink?> resolveExternalLink(String input) async {
  final trimmed = input.trim();
  final resolved = await _resolveDeepLink(trimmed);
  if (resolved != null) return resolved;
  if (isShareLinkInput(trimmed)) {
...
Future<ResolvedExternalLink?> _resolveDeepLink(String trimmed) async {
  if (trimmed.isEmpty) return null;

  final embedded = _findEmbeddedLink(trimmed);
  if (embedded != null) return _resolveDeepLink(embedded);
...
String? _findEmbeddedLink(String input) {
  ...
  for (final scheme in ['incy://', 'happ://']) {
    ...
    if (idx >= 0) {
      ...
      return decoded.substring(idx).replaceAll(RegExp(r'[)>\]]+$'), '').trim();
    }
  }
  return null;
}
...
Future<ResolvedExternalLink?> _resolveIncy(String link) async {
  ...
  for (final kind in ['add/', 'import/']) {
    if (path.startsWith(kind)) {
      return _resolveWrapped(
        path.substring(kind.length),
...
Future<ResolvedExternalLink> _resolveWrapped(
  String value,
  SubscriptionClient preset,
) async {
  final payload = _decodeComponent(value);
  final inner = await _resolveDeepLink(payload);
  if (inner != null) return inner;
  final isUrl = payload.startsWith('http://') || payload.startsWith('https://');
  return ResolvedExternalLink(
    url: isUrl ? payload : '',
    data: isUrl ? null : payload,
    preset: preset,
  );
}
