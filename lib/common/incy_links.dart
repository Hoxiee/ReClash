/// `incy://` and `happ://` deep links, resolved locally: an AES-256-GCM
/// obfuscation of a subscription URL, keyed per the `@incy/link-encoder`
/// reference (MIT) and checked against the published fingerprint.
library;
...
class ResolvedExternalLink {
  ...
  final SubscriptionClient preset;
...
class IncyLinkException implements Exception {
  const IncyLinkException(this.message);
...
List<int>? _tryB64UrlDecode(String input) {
...
List<int>? _derivedKey;

/// K1 = SHA-256(salt parts || keymatA[1024:1056] || keymatB[2048:2080]).
List<int> _crypt1Key() {
  if (_derivedKey != null) return _derivedKey!;
  final a = base64.decode(_keymatA);
  final b = base64.decode(_keymatB);
  final seed = utf8.encode('incy') +
      utf8.encode('deep') +
      utf8.encode('crypt1') +
      utf8.encode('v2026.06') +
      a.sublist(1024, 1024 + 32) +
      b.sublist(2048, 2048 + 32);
  final key = sha256.convert(seed).bytes;
  final fingerprint = sha256.convert(key).toString();
  if (fingerprint != _expectedKeyFingerprint) {
    throw const IncyLinkException(
      'incy crypt1 key fingerprint mismatch — keymat out of sync',
    );
  }
  return _derivedKey = key;
}

Future<({String url, String? name})> _decryptCrypt1(String link) async {
  if (!link.startsWith(_linkPrefix)) {
    throw const IncyLinkException('not an incy://crypt1 link');
  }
  var payload = link.substring(_linkPrefix.length).replaceAll(RegExp(r'/+$'), '');
  // Some share channels append junk separators — stop at the first space.
  final space = payload.indexOf(' ');
  if (space > 0) payload = payload.substring(0, space);
  final wire = _tryB64UrlDecode(payload);
  if (wire == null || wire.length < _ivLen + _tagLen + 1) {
    throw const IncyLinkException('incy crypt1 payload is malformed');
  }

  final box = SecretBox(
    wire.sublist(_ivLen, wire.length - _tagLen),
    nonce: wire.sublist(0, _ivLen),
    mac: Mac(wire.sublist(wire.length - _tagLen)),
  );
  List<int> clear;
  try {
    clear = await AesGcm.with256bits().decrypt(
      box,
      secretKey: SecretKey(_crypt1Key()),
    );
  } on SecretBoxAuthenticationError {
    throw const IncyLinkException('incy crypt1 authentication failed');
  } catch (_) {
    throw const IncyLinkException('incy crypt1 decryption failed');
  }

  Map<String, Object?> parsed;
  try {
    parsed = jsonDecode(utf8.decode(clear)) as Map<String, Object?>;
  } catch (_) {
    throw const IncyLinkException('incy crypt1 plaintext is malformed');
  }
  final url = parsed['url'];
  if (url is! String || url.isEmpty) {
    throw const IncyLinkException('incy crypt1 plaintext has no url');
  }
  final name = parsed['n'];
  return (url: url, name: name is String && name.isNotEmpty ? name : null);
}

/// Unwrap a deep link (or an http(s) URL embedding one). Null when [input] is
/// not a deep link; throws [IncyLinkException] on unresolvable ones.
Future<ResolvedExternalLink?> resolveExternalLink(String input) async {
  final trimmed = input.trim();
  if (trimmed.isEmpty) return null;

  final embedded = _findEmbeddedLink(trimmed);
  if (embedded != null) return resolveExternalLink(embedded);

  if (trimmed.startsWith('incy://')) return _resolveIncy(trimmed);
  if (trimmed.startsWith('happ://')) return _resolveHapp(trimmed);
  return null;
}

String? _findEmbeddedLink(String input) {
  ...
  for (final scheme in ['incy://', 'happ://']) {
    final idx = decoded.indexOf(scheme);
    if (idx >= 0) {
      // Trailing URL punctuation that share-texts accumulate after a link.
      return decoded
          .substring(idx)
          .replaceAll(RegExp(r'[)>\]]+$'), '')
          .trim();
...
Future<ResolvedExternalLink?> _resolveIncy(String link) async {
  final path = link.substring('incy://'.length);

  if (path.startsWith('crypt1/')) {
    final decrypted = await _decryptCrypt1(link);
    return ResolvedExternalLink(
      url: decrypted.url,
      name: decrypted.name,
      preset: SubscriptionClient.incy,
    );
  }
  if (path.startsWith('crypt')) {
    // crypt2+ would be a key rotation we do not have the keymat for.
    throw const IncyLinkException('unsupported incy crypto scheme');
  }
  if (path.startsWith('add/')) {
    final target = _decodeComponent(path.substring('add/'.length));
    final inner = await resolveExternalLink(target);
  ...
  if (path.startsWith('import/')) {
    final payload = _decodeComponent(path.substring('import/'.length));
    final inner = await resolveExternalLink(payload);
    if (inner != null) return inner;
  ...
  throw const IncyLinkException('unknown incy link kind');
}

Future<ResolvedExternalLink?> _resolveHapp(String link) async {
  final path = link.substring('happ://'.length);
  if (path.startsWith('crypt')) {
    // Happ's own encrypted links (crypt2-5) are out of scope.
    throw const IncyLinkException('happ crypto links are not supported');
  }
  if (path.startsWith('add/')) {
    final target = _decodeComponent(path.substring('add/'.length));
    final inner = await resolveExternalLink(target);
    ...
    return ResolvedExternalLink(
      url: target,
      preset: SubscriptionClient.happ,
    );
  }
  throw const IncyLinkException('unknown happ link kind');
}
