...
void main() {
  ...
  group('tryConvertSingboxConfig', () {
    test('vless with reality + utls + grpc', () {
      final SingboxConfigResult? result = tryConvertSingboxConfig(
        _json({
          'outbounds': [
            outbound(
              'vless',
      ...
      expect(
        config,
        contains('reality-opts: {public-key: "PUBKEY", short-id: "e6c87ce"}'),
      );
    ...
    test('socks5 drops the SNI field entirely', () {
      final result = tryConvertSingboxConfig(
        _json({
          'outbounds': [
            outbound('socks', 'SK', extra: {'version': '5'}, tls: tls({})),
          ],
        }),
      );
      expect(result, isNotNull);
    ...
    test('ss plugin opts pass through as a map or SIP003 string', () {
      ...
      expect(config, contains('plugin-opts: {mode: "websocket", path: "/v"}'));
        ...
        _json({
          'outbounds': [
            ...
            outbound(
              'hysteria2',
              'HY2-ok',
              extra: {'password': 'pw', 'up_mbps': 30, 'down_mbps': 100},
            ),
          ],
        }),
      );
        ...
        _json({
          'outbounds': [
            ...
            outbound(
              'hysteria',
              'hy1-bad-obfs',
              extra: {'auth_str': 'a', 'obfs': 'salamander'},
            ),
            outbound(
              'vless',
              'bad-transport',
