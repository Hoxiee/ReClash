...
void main() {
  ...
  test(
    'makeRealProfileTask keeps provider network values by default',
    () async {
      ...
      // The app always owns the transparent-redirect ports.
      expect(kept['redir-port'], 0);
      expect(kept['tproxy-port'], 0);
      ...
      expect(overridden['redir-port'], 0);
      expect(overridden['tproxy-port'], 0);
    },
  );

  test(
    'makeRealProfileTask falls back to the app UA when default is chosen',
    () async {
      final result = await makeRealProfileTask(
        MakeRealProfileState(
          profilesPath: '/profiles',
  ...
  test(
    'makeRealProfileTask fills missing network keys from the patch',
    () async {
      final rawConfig = await decodeJSONTask<Map<String, dynamic>>(
